...
class CubeScene:
    ...
    def set_effects(self, shadows=None, ssao=None, fxaa=None, ordering=None):
        ...
        try:
            if self.use_shadows:
                ...
                # VTK complains loudly if a render pass is dropped without
                # releasing its framebuffer and shadow maps first.
    ...
    def set_atom_scheme(self, scheme):
        """
        Atom colouring. "element" uses VTK's periodic-table colours; the
        others override every atom with one tone, which reads better when the
        surface is the subject and the molecule is only a reference.
        """
        ...
        prop = self._mol_actor.GetProperty()
        flat = {"mono": (0.78, 0.78, 0.82),
                "warm": (0.85, 0.62, 0.42),
                "cool": (0.55, 0.72, 0.88)}.get(self.atom_scheme)
        try:
            if flat is None:
                mapper.SetAtomColorMode(1)      # per-element
                prop.SetColor(1.0, 1.0, 1.0)
            else:
                mapper.SetAtomColorMode(0)      # single colour
                mapper.SetAtomColor(*[int(c * 255) for c in flat])
                prop.SetColor(*flat)
        except Exception:
            prop.SetColor(*(flat or (1.0, 1.0, 1.0)))
